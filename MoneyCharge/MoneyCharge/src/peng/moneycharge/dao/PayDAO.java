...
public class PayDAO {
	...
	public void update(Tb_pay tb_pay) {
		...
		db.execSQL(
				...
				new Object[] { tb_pay.getMoney(), tb_pay.getTime(),
						...
						tb_pay.getMark(), tb_pay.get_id(), tb_pay.getNo() });
	}
	...
	public List<Tb_pay> getScrollData(int id, int start, int count) {
		...
		Cursor cursor = db
				.rawQuery(
						"select _id,no,money,time,type,address,mark from tb_pay where _id=? order by no limit ?,?",
