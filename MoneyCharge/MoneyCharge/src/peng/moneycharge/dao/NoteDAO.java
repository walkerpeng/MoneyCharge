...
public class NoteDAO {
	...
	public void add(Tb_note tb_note) {
		...
		db.execSQL(
				"insert into tb_note (_id,no,note) values (?,?,?)",
				new Object[] { tb_note.get_id(), tb_note.getNo(),
						tb_note.getNote() });// 执行添加便签信息操作
	}
	...
	public void update(Tb_note tb_note) {
		db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
		db.execSQL(
				"update tb_note set note = ? where _id = ? and no=?",
				new Object[] { tb_note.getNote(), tb_note.get_id(),
						tb_note.getNo() });// 执行修改便签信息操作
	}
	...
	public List<Tb_note> getScrollData(int id, int start, int count) {
		...
		Cursor cursor = db
				.rawQuery(
						"select _id,no,note from tb_note where _id=? order by no limit ?,?",
						new String[] { String.valueOf(id),
								String.valueOf(start), String.valueOf(count) });
		while (cursor.moveToNext())// 遍历所有的便签信息
		{
			// 将遍历到的便签信息添加到集合中
			lisTb_note.add(new Tb_note(cursor.getInt(cursor
	...
	public long getCount(int id) {
		...
		Cursor cursor = db.rawQuery(
				...
				new String[] { String.valueOf(id) });// 获取便签信息的记录数
	...
	public int getMaxNo(int id) {
		db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
		Cursor cursor = db.rawQuery("select max(no) from tb_note where _id=?",
