...
import peng.moneycharge.model.Tb_itype;
...
public class ItypeDAO {
	...
	public void add(Tb_itype tb_itype) {
		...
		db.execSQL(
				"insert into tb_itype (_id,no,typename) values (?,?,?)",
				new Object[] { tb_itype.get_id(), tb_itype.getNo(),
						tb_itype.getTypename() });// 执行添加收入类型操作
	...
	public void modify(Tb_itype tb_itype) {
		db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
		db.execSQL(
				"update tb_itype set typename = ? where _id = ? and no=?",
				new Object[] { tb_itype.get_id(), tb_itype.getNo(),
						tb_itype.getTypename() });// 执行修改收入类型操作
	}
	...
	public void deleteByName(int id, String typename) {
		db = helper.getWritableDatabase();// 创建SQLiteDatabase对象
		// 执行删除便签信息操作
		db.execSQL("delete from tb_itype where _id =? and typename=?",
	...
	public List<Tb_itype> getScrollData(int id, int start, int count) {
		...
		Cursor cursor = db.rawQuery(
				"select * from tb_itype where _id=? order by no limit ?,?",
				new String[] { String.valueOf(id), String.valueOf(start),
						String.valueOf(count) });
		while (cursor.moveToNext())// 遍历所有的便签信息
		{
			// 将遍历到的便签信息添加到集合中
			lisTb_itype.add(new Tb_itype(cursor.getInt(cursor
	...
	public List<String> getItypeName(int id) {
		...
		Cursor cursor = db.rawQuery(
				"select typename from tb_itype where _id=?",
	...
	public String getOneName(int id, int no) {
		db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
		Cursor cursor = db.rawQuery(
				"select typename from tb_itype where _id=? and no=?",
	...
	public List<Datapicker> getDataOnDay(int id, String date1, String date2) {
		...
		Cursor cursor = db.rawQuery(
				"select no, time,sum(money) as tmoney from tb_pay where time between "
						+ sql + " and _id =? group BY time order by time",
				new String[] { String.valueOf(id) });// 获取支出信息表中的最大编号
		while (cursor.moveToNext())// 遍历所有的支出信息
		{

			// 将遍历到的支出信息添加到集合中
			datapicker.add(new Datapicker(cursor.getInt(cursor
					.getColumnIndex("no")), cursor.getDouble(cursor
					.getColumnIndex("tmoney")), cursor.getString(cursor
					.getColumnIndex("time"))));
		}
	...
	public void modifyByName(int id, String old, String now) {
		db = helper.getWritableDatabase();
		db.execSQL(
				"update tb_itype set typename = ? where _id = ? and typename=?",
	...
	public void initData(int id) {
		...
		// 便签信息表
		db.execSQL("delete from tb_itype where _id=?",
				new String[] { String.valueOf(id) });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "1", "工资" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "2", "还款" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "3", "股票" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "4", "还款" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "5", "基金" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "6", "分红" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "7", "利息" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "8", "兼职" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "9", "奖金" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "10", "租金" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "11", "销售款" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "12", "应收款" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "13", "报销款" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "14", "其他" });
		db.execSQL("insert into tb_itype(_id,no,typename) values(?,?,?)",
				new String[] { String.valueOf(id), "15", "礼金" });
	}
}
