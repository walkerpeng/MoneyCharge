...
import peng.moneycharge.model.KindData;
...
public class IncomeData extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		adapter = new ArrayAdapter<String>(IncomeData.this,
	...
	protected void onStart() {
		 ...
		 * mRenderer.setChartTitleTextSize(20); mRenderer.setLabelsTextSize(20);
		 * mRenderer.setLabelsColor(Color.BLACK); mRenderer.setShowAxes(false);
		 * // 是否显示轴线 // mRenderer.setAxesColor(Color.RED); //设置轴颜色
		 * mRenderer.setFitLegend(false); mRenderer.setInScroll(false);
		 * mRenderer.setPanEnabled(false);
		 * mRenderer.setShowCustomTextGrid(false);
		 * mRenderer.setShowLegend(false); //不显示图例 mRenderer.setShowGrid(false);
		 * mRenderer.setClickEnabled(true); // mRenderer.setScale(1.5f); //
		 * mRenderer.setLegendTextSize(15); // mRenderer.setMargins(new int[] {
		 * 20, 50, 15, 0 }); // mRenderer.setZoomButtonsVisible(true);
		...
		if (type == 0) {
			KindDatai = incomeDAO.getKDataOnMonth(userid, defaultYear,
					defaultMonth);
		...
		if (KindDatai.size() == 0) {
			...
			for (KindData ki : KindDatai)
				sum += ki.getAmount();// 总和
			for (KindData ki : KindDatai) {
				mSeries.add(itypeDAO.getOneName(userid, ki.getKindname()),
						ki.getAmount() / sum);
		...
		beforet.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intentp.putExtra("grk.id", userid);
