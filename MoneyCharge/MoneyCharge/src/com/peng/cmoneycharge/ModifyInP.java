...
public class ModifyInP extends Activity {
	...
	protected void onStart() {
		...
		if (strType.equals("btnoutinfo"))// 如果类型是btnoutinfo
		{
			...
			adapter = new ArrayAdapter<String>(ModifyInP.this,
		...
		btnEdit.setOnClickListener(new OnClickListener() {// 为修改按钮设置监听事件
			...
			public void onClick(View arg0) {
				...
				if (strType.equals("btnoutinfo"))// 判断类型如果是btnoutinfo
				{
					...
					tb_pay.setMoney(Double.parseDouble(txtMoney.getText()
							.toString()));// 设置金额
					...
					Toast.makeText(ModifyInP.this, "〖数据〗修改成功！",
				...
				} else if (strType.equals("btnininfo"))// 判断类型如果是btnininfo
				{
					...
					tb_income.setMoney(Double.parseDouble(txtMoney.getText()
							.toString()));// 设置金额
					tb_income.setTime(setTimeFormat(txtTime.getText()
							.toString()));// 设置时间
